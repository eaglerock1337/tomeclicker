import { o as De, g as wt } from './Bl-oD2Vi.js';
import { bp as me, aW as vt, aR as U, h as L, Q as P, aO as qe } from './BGLDHNfr.js';
const q = [];
function Re(e, t = me) {
	let n = null;
	const r = new Set();
...
const It = window.fetch;
window.fetch = (e, t) => (
	(e instanceof Request ? e.method : t?.method || 'GET') !== 'GET' && K.delete(Ue(e)),
	It(e, t)
);
const K = new Map();
function Ut(e, t) {
	...
	if (r?.textContent) {
		...
		return (
			i && K.set(n, { body: a, init: s, ttl: 1e3 * Number(i) }),
...
function jt({ nodes: e, server_loads: t, dictionary: n, matchers: r }) {
	const a = new Set(t);
	return Object.entries(n).map(([o, [c, l, d]]) => {
		const { pattern: p, params: u } = xt(o),
			f = {
				id: o,
				exec: (h) => {
					const w = p.exec(h);
					if (w) return Ot(w, u, r);
				},
...
const I = globalThis.__sveltekit_1oeqibf?.base ?? '/tomeclicker',
	$t = globalThis.__sveltekit_1oeqibf?.assets ?? I ?? '',
	Dt = '1761048166046',
...
function ke(e, t, n) {
	...
	const a = e instanceof SVGAElement ? e.target.baseVal : e.target,
		s = !r || !!a || de(r, t, n) || (e.getAttribute('rel') || '').split(/\s+/).includes('external'),
...
function Ft() {
	...
	async function r() {
		clearTimeout(n);
		try {
			const a = await fetch(`${$t}/_app/version.json`, {
...
const at = new Set(['load', 'prerender', 'csr', 'ssr', 'trailingSlash', 'config']);
[...at];
const Xt = new Set([...at]);
[...Xt];
function Zt(e) {
...
function ne(e) {
	return e instanceof le || e instanceof Ie ? e.status : 500;
}
function tn(e) {
	return e instanceof Ie ? e.text : 'Internal Error';
}
let A, J, ve;
const nn = De.toString().includes('$$') || /function \w+\(\) \{\}/.test(De.toString());
nn
	? ((A = {
			data: {},
			form: null,
...
const an = '/__data.json',
	on = '.html__data.json';
function sn(e) {
	return e.endsWith('.html') ? e.replace(/\.html$/, on) : e.replace(/\/$/, '') + an;
}
const Me = {
		spanContext() {
			return cn;
	...
	cn = { traceId: '', spanId: '', traceFlags: 0 },
	{ tick: ln } = wt,
	fn = new Set(['icon', 'shortcut icon', 'apple-touch-icon']),
	D = Je(Ze) ?? {},
	X = Je(Xe) ?? {},
	O = { url: Ve({}), page: Ve({}), navigating: Re(null), updated: Ft() };
function Le(e) {
	D[e] = ue();
}
function un(e, t) {
...
async function xn(e, t, n) {
	(globalThis.__sveltekit_1oeqibf?.data && globalThis.__sveltekit_1oeqibf.data,
		...
		(S = history.state?.[B]),
		(T = history.state?.[z]),
		S || ((S = T = Date.now()), history.replaceState({ ...history.state, [B]: S, [z]: T }, '')));
	const r = D[S];
	function a() {
	...
	(n
		...
		: (await V({
				type: 'enter',
				url: et(v.hash ? An(new URL(location.href)) : location.href),
				replace_state: !0
			}),
			a()),
		kn());
...
function lt(e) {
	oe.some((t) => t?.snapshot) && (X[e] = oe.map((t) => t?.snapshot?.capture()));
}
function ft(e) {
	X[e]?.forEach((t, n) => {
...
async function pn(e, t, n, r) {
	...
	(t.invalidateAll && (C = null),
		await V({
			...
			nav_token: r,
			accept: () => {
				(t.invalidateAll && ((Ce = !0), (a = [...Ke.keys()])),
					t.invalidate && t.invalidate.forEach(bn));
			}
		}),
		t.invalidateAll &&
			qe()
		...
		(Y.add(t),
			(C = {
				...
				promise: ht({ ...e, preload: t }).then(
					(n) => (Y.delete(t), n.type === 'loaded' && n.state.error && (C = null), n)
				)
			}));
	}
	return C.promise;
...
function ie({ url: e, params: t, branch: n, status: r, error: a, route: s, form: i }) {
	...
	const c = {
		...
		props: { constructors: Zt(n).map((f) => f.node.component), page: $e(A) }
	};
	...
	return (
		(!y.url || e.href !== y.url.href || y.error !== a || (i !== void 0 && i !== A.form) || d) &&
			(c.props.page = {
				error: a,
				params: t,
				route: { id: s?.id ?? null },
...
async function Ne({ loader: e, parent: t, url: n, params: r, route: a, server_data_node: s }) {
	...
	if (l.universal?.load) {
		...
		const p = {
			tracing: { enabled: !1, root: Me, current: Me },
			...
			async fetch(u, f) {
				...
				const { resolved: h, promise: w } = dt(u, f, n);
				return (o && d(h.href), w);
			},
			setHeaders: () => {},
			depends: d,
...
function dt(e, t, n) {
	...
	const s = se ? Lt(r, a.href, t) : Ut(r, t);
...
function ze({ error: e, url: t, route: n, params: r }) {
	return {
		...
		props: { page: $e(A), constructors: [] }
	};
}
async function ht({ id: e, invalidating: t, url: n, params: r, route: a, preload: s }) {
	if (C?.id === e) return (Y.delete(C.token), C.promise);
	const { errors: i, layouts: o, leaf: c } = a,
		l = [...o, c];
	(i.forEach((m) => m?.().catch(() => {})), l.forEach((m) => m?.[1]().catch(() => {})));
	let d = null;
	...
	const w = l.map((m, g) => {
		const b = y.branch[g],
			k = !!m?.[0] && (b?.loader !== m[1] || Ye(h, u, p, f, b.server?.uses, r));
		return (k && (h = !0), k);
	});
	if (w.some(Boolean)) {
		try {
			...
			const g = await M(m, { url: n, params: r, route: { id: e } });
			return Y.has(s)
				? ze({ error: g, url: n, params: r, route: a })
				: he({ status: ne(m), error: g, url: n, route: a });
		}
		if (d.type === 'redirect') return d;
	}
	const E = d?.nodes;
	let _ = !1;
	const N = l.map(async (m, g) => {
		if (!m) return;
		const b = y.branch[g],
			k = E?.[g];
		if ((!k || k.type === 'skip') && m[1] === b?.loader && !Ye(_, u, p, f, b.universal?.uses, r))
			return b;
		if (((_ = !0), k?.type === 'error')) throw k;
		return Ne({
			loader: m[1],
			url: n,
			params: r,
			...
			parent: async () => {
				const ge = {};
				for (let _e = 0; _e < g; _e += 1) Object.assign(ge, (await N[_e])?.data);
				return ge;
			},
			server_data_node: Oe(
	...
	for (let m = 0; m < l.length; m += 1)
		if (l[m])
			try {
				...
				if (g instanceof Te) return { type: 'redirect', location: g.location };
				if (Y.has(s))
					return ze({
						error: await M(g, { params: r, url: n, route: { id: a.id } }),
						url: n,
						params: r,
						route: a
					});
				let b = ne(g),
					k;
				if (E?.includes(g)) ((b = g.status ?? b), (k = g.error));
				else if (g instanceof le) k = g.body;
				else {
					if (await O.updated.check()) return (await ot(), await G(n));
					k = await M(g, { params: r, url: n, route: { id: a.id } });
				}
				const Q = await mn(m, R, i);
				return Q
					? ie({
							url: n,
							params: r,
							branch: R.slice(0, Q.idx).concat(Q.node),
							status: b,
							error: k,
							route: a
						})
					: await gt(n, { id: a.id }, k, b);
			}
		else R.push(void 0);
	return ie({
		url: n,
...
async function he({ status: e, error: t, url: n, route: r }) {
	...
	if (v.server_loads[0] === 0)
		try {
			...
			(n.origin !== fe || n.pathname !== location.pathname || xe) && (await G(n));
		}
	try {
		const o = await Ne({
				loader: Ee,
		...
		if (o instanceof Te) return pn(new URL(o.location, location.href), {}, 0);
		throw o;
	}
}
async function yn(e) {
	...
	try {
		const r = (async () => {
			let a =
				(await v.hooks.reroute({ url: new URL(e), fetch: async (s, i) => dt(s, i, e).promise })) ??
				e;
			if (typeof a == 'string') {
				const s = new URL(e);
...
function wn(e) {
	return (
		kt(v.hash ? e.hash.replace(/^#/, '').replace(/[?#].+/, '') : e.pathname.slice(I.length)) || '/'
	);
}
function ce(e) {
...
function pt({ url: e, type: t, intent: n, delta: r, event: a }) {
	...
	return (Z || st.forEach((c) => c(o)), s ? null : i);
}
async function V({
	type: e,
	url: t,
	...
	const u = $;
	$ = c;
	const f = await pe(t, !1),
		h =
			e === 'enter'
				? je(y, f, t, e)
				: pt({ url: t, type: e, delta: n?.delta, intent: f, event: p });
	if (!h) {
		(d(), $ === c && ($ = u));
		return;
	}
	...
	(l(),
		(Z = !0),
		se && h.navigation.type !== 'enter' && O.navigating.set((J.current = h.navigation)));
	let _ = f && (await ht(f));
	if (!_) {
		if (de(t, I, v.hash)) return await G(t, s);
		_ = await gt(
			t,
			{ id: null },
			await M(new Ie(404, 'Not Found', `Not found: ${t.pathname}`), {
				url: t,
				params: {},
				route: { id: null }
			}),
	...
	if (((t = f?.url || t), $ !== c)) return (h.reject(new Error('navigation aborted')), !1);
	if (_.type === 'redirect') {
		if (o < 20) {
			(await V({
				type: e,
	...
	} else _.props.page.status >= 400 && (await O.updated.check()) && (await ot(), await G(t, s));
	if (
		(hn(),
		Le(w),
		lt(E),
		...
		const g = s ? 0 : 1,
			b = { [B]: (S += g), [z]: (T += g), [Qe]: i };
		((s ? history.replaceState : history.pushState).call(history, b, '', t), s || un(S, T));
	}
	if (((C = null), (_.props.page.state = i), se)) {
		const g = (await Promise.all(Array.from(dn, (b) => b(h.navigation)))).filter(
			(b) => typeof b == 'function'
		);
		if (g.length > 0) {
			let b = function () {
	...
	await ln();
	const R = n ? n.scroll : a ? ue() : null;
	if (He) {
		const g = t.hash && document.getElementById(yt(t));
		R ? scrollTo(R.x, R.y) : g ? g.scrollIntoView() : scrollTo(0, 0);
	}
	const m = document.activeElement !== N && document.activeElement !== document.body;
	(!r && !m && Sn(t),
		(He = !0),
		_.props.page && Object.assign(A, _.props.page),
		(Z = !1),
		e === 'popstate' && ft(T),
		h.fulfil(void 0),
		W.forEach((g) => g(h.navigation)),
		O.navigating.set((J.current = null)));
}
async function gt(e, t, n, r, a) {
	return e.origin === fe && e.pathname === location.pathname && !xe
...
function vn() {
	...
	async function s(o, c) {
		const l = nt(o, x),
			d = l === t && c >= n;
		if (!l || d) return;
		const { url: p, external: u, download: f } = ke(l, I, v.hash);
		if (u || f) return;
		const h = te(l),
			w = p && ce(y.url) === ce(p);
		if (!(h.reload || w))
			if (c <= h.preload_data) {
				((t = l), (n = j.tap));
				const E = await pe(p, !1);
				if (!E) return;
				gn(E);
			} else c <= h.preload_code && ((t = l), (n = c), be(p));
	}
	function i() {
		a.disconnect();
		for (const o of x.querySelectorAll('a')) {
			const { url: c, external: l, download: d } = ke(o, I, v.hash);
			if (l || d) continue;
			const p = te(o);
			p.reload ||
				(p.preload_code === j.viewport && a.observe(o), p.preload_code === j.eager && be(c));
		}
	}
	(W.add(i), i());
}
...
function kn() {
	((history.scrollRestoration = 'manual'),
		addEventListener('beforeunload', (t) => {
			let n = !1;
			if ((We(), !Z)) {
				const r = je(y, void 0, null, 'leave'),
					a = {
						...r.navigation,
						cancel: () => {
		...
		navigator.connection?.saveData || vn(),
		x.addEventListener('click', async (t) => {
			if (
				...
				return;
			const n = nt(t.composedPath()[0], x);
			if (!n) return;
			const { url: r, external: a, target: s, download: i } = ke(n, I, v.hash);
			if (!r) return;
			if (s === '_parent' || s === '_top') {
			...
			const [l, d] = (v.hash ? r.hash.replace(/^#/, '') : r.href).split('#'),
				p = l === ye(location);
			if (a || (o.reload && (!p || !d))) {
				pt({ url: r, type: 'link', event: t }) ? (Z = !0) : t.preventDefault();
				return;
			}
			if (d !== void 0 && p) {
				...
				if (((H = !0), Le(S), e(r), !o.replace_state)) return;
				H = !1;
			}
			(t.preventDefault(),
				await new Promise((u) => {
		...
		addEventListener('popstate', async (t) => {
			if (!Ae) {
				if (t.state?.[B]) {
					...
					if (i === T && (it || o)) {
						(a !== A.state && (A.state = a), e(s), (D[S] = ue()), r && scrollTo(r.x, r.y), (S = n));
						return;
					}
					const l = n - S;
					await V({
		...
		addEventListener('hashchange', () => {
			H &&
				((H = !1), history.replaceState({ ...history.state, [B]: ++S, [z]: T }, '', location.href));
		}));
	for (const t of document.querySelectorAll('link')) fn.has(t.rel) && (t.href = t.href);
	addEventListener('pageshow', (t) => {
		t.persisted && O.navigating.set((J.current = null));
	});
	function e(t) {
		((y.url = A.url = t), O.page.set($e(A)), O.page.notify());
	}
}
async function En(
	e,
	...
	(({ params: a = {}, route: s = { id: null } } = (await pe(l, !1)) || {}),
		(d = Pe.find(({ id: f }) => f === s.id)));
	let p,
		u = !0;
	try {
		const f = r.map(async (w, E) => {
				const _ = o[E];
				return (
					_?.uses && (_.uses = mt(_.uses)),
					Ne({
						loader: v.nodes[w],
						url: l,
						params: a,
						...
						parent: async () => {
							const N = {};
							for (let R = 0; R < E; R += 1) Object.assign(N, (await f[R]).data);
							return N;
						},
						server_data_node: Oe(_)
					})
				);
			}),
		...
		p = ie({ url: l, params: a, branch: h, status: t, error: n, form: c, route: d ?? null });
	} catch (f) {
		if (f instanceof Te) {
	...
	(p.props.page && (p.props.page.state = {}), ut(p, e, u));
}
async function _t(e, t) {
	const n = new URL(e);
	((n.pathname = sn(e.pathname)),
		e.pathname.endsWith('/') && n.searchParams.append(en, '1'),
		n.searchParams.append(Qt, t.map((s) => (s ? '1' : '0')).join('')));
...
`);
				...
				if (f.type === 'data')
					(f.nodes?.forEach((h) => {
						h?.type === 'data' && ((h.uses = mt(h.uses)), (h.data = c(h.data)));
					}),
						s(f));
				else if (f.type === 'chunk') {
					const { id: h, data: w, error: E } = f,
						_ = i.get(h);
...
export { xn as a, In as l, A as p, O as s };
