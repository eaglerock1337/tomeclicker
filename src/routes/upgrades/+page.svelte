<script lang="ts">
	...
	let selectedUpgrade: Upgrade | null = $state(null);
	...
	function purchaseSelected() {
		if (selectedUpgrade && game.purchaseUpgrade(selectedUpgrade.id)) {
			...
			// Save progress
			game.autoSave();
			gameStore.set(game); // Update store
		}
	}
	...
	let availableUpgrades = $derived(
		Object.values(game.upgrades)
			.filter((u) => !u.minLevel || u.minLevel <= game.level)
			.filter((u) => u.id !== 'discipline')
	);
	let discipline = $derived(game.upgrades['discipline']);
	...
	<div class="upgrades-layout">
		<div class="upgrade-grid">
			...
			<button
				...
				aria-label="Level up from {game.level} to {game.level +
					1}. Cost: {formatCost(game.getLevelUpCost())} EXP"
			>
			...
			<button
				...
				onclick={() => selectUpgrade(discipline)}
				aria-label="Select {discipline.name} upgrade. Level {discipline.currentLevel} of {discipline.maxLevel}. Cost: {formatCost(
					game.getUpgradeCost(discipline.id)
				)} EXP"
			>
			...
			{#each availableUpgrades as upgrade (upgrade.id)}
				<button
					...
					aria-label="Select {upgrade.name} upgrade. Level {upgrade.currentLevel} of {upgrade.maxLevel}. Cost: {formatCost(
			...
			{#if selectedUpgrade}
				...
				<div class="details-body">
					<div class="details-content">
						...
						<p class="cost">
							<strong>Cost:</strong>
							{formatCost(game.getUpgradeCost(selectedUpgrade.id))} EXP
						</p>
						<p class="level">
							<strong>Level:</strong>
